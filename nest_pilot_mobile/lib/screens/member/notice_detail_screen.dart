...
class _NoticeDetailScreenState extends State<NoticeDetailScreen> {
  ...
  /// Deletes the notice and pops `true` so the list knows to refetch.
  Future<void> _confirmDelete() async {
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
                  'Delete "${widget.notice.title}"? This cannot be undone.',
    ...
    try {
      final ok = await _service.deleteNotice(widget.notice.id);
      if (!mounted) return;
      if (ok) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Failed to delete notice')));
    } catch (e) {
      if (!mounted) return;
      setState(() => _deleting = false);
      ScaffoldMessenger.of(
  ...
  Widget build(BuildContext context) {
    ...
    // `manage` is the app's delete gate — see PermissionService: it covers
    // add / edit / delete / approve. There is no separate delete permission.
    final canDelete = PermissionService().canManage(ModuleCodes.notices);
              ...
              child: DecoratedBox(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(_deleting ? 'Deleting…' : 'Delete Notice'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: AppColors.accentRed,
                      foregroundColor: AppColors.white,
