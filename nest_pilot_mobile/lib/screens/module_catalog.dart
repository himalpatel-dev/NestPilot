...
// canManage(AMENITIES) lands on AmenityManagementScreen, everyone else on
// AmenityBookingScreen. Add a tile here once and it shows for any role whose
...
List<ModuleSection> filterModuleSections(List<ModuleSection> sections) {
  ...
  for (final s in sections) {
    final allowed = s.tiles
        .where(
          (t) => t.module == null || perms.can(t.module!, t.requiredAction),
        )
        .toList();
...
class _ModuleTileView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AppIconTile(icon: tile.icon, color: tile.color, label: tile.label),
    );
  }
}
